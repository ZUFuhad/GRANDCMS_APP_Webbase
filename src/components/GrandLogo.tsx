      ...
      >
        ...
        <g transform="translate(14, 42)">
          ...
          <path
            ...
            fill={variant === 'gold' ? 'url(#grandGoldGradient)' : primaryColor}
          />
          <rect x="36" y="72" width="10" height="36" rx="1.5" fill={variant === 'gold' ? 'url(#grandCrownGradient)' : primaryColor} />
          <rect x="50" y="52" width="10" height="56" rx="1.5" fill={variant === 'gold' ? 'url(#grandCrownGradient)' : primaryColor} />
          <rect x="64" y="36" width="10" height="72" rx="1.5" fill={variant === 'gold' ? 'url(#grandCrownGradient)' : primaryColor} />
        </g>

        {/* --- 'RAND' --- */}
        <g transform="translate(108, 52)">
          <text x="0" y="85" fontFamily="'Cinzel', Georgia, serif" fontSize="92" fontWeight="800" letterSpacing="-2" fill={variant === 'gold' ? 'url(#grandGoldGradient)' : primaryColor}>R</text>
          <text x="62" y="85" fontFamily="'Cinzel', Georgia, serif" fontSize="92" fontWeight="800" letterSpacing="-2" fill={variant === 'gold' ? 'url(#grandGoldGradient)' : primaryColor}>A</text>
          <text x="126" y="85" fontFamily="'Cinzel', Georgia, serif" fontSize="92" fontWeight="800" letterSpacing="-2" fill={variant === 'gold' ? 'url(#grandGoldGradient)' : primaryColor}>N</text>
          <text x="188" y="85" fontFamily="'Cinzel', Georgia, serif" fontSize="92" fontWeight="800" letterSpacing="-2" fill={variant === 'gold' ? 'url(#grandGoldGradient)' : primaryColor}>D</text>
        </g>

        {/* --- TAGLINE & EST 2004 --- */}
        {showTagline && (
          <>
            <text x="160" y="156" textAnchor="middle" fontFamily="'Dancing Script', cursive" fontSize="21" fontWeight="700" fill={primaryColor} letterSpacing="0.8">
              we value what you have to say!
            </text>
            <line x1="18" y1="172" x2="110" y2="172" stroke={primaryColor} strokeWidth="0.8" opacity="0.6" />
            <text x="160" y="175" textAnchor="middle" fontFamily="'Cinzel', sans-serif" fontSize="12.5" fontWeight="700" letterSpacing="5" fill={primaryColor}>
              EST. 2004
            </text>
            <line x1="210" y1="172" x2="302" y2="172" stroke={primaryColor} strokeWidth="0.8" opacity="0.6" />
          </>
        )}
      </svg>
    </div>
